...
def read_midi_roll(
    ...
    """
    Parse MIDI → beat-based note list + segment boundaries for piano-roll rendering.

    Segment labeling strategy (first match wins):
      1. MusicXML rehearsal marks via get_musicxml_sections
      2. n_variations parameter
      3. Annotation Excel (K-number lookup)
      4. Fallback: 16-bar theme + 12 × 16-bar variations
    """
    if mido is None:
    ...
    if not var_labels and k_num and ANNOTATION.exists():
        try:
            ...
            if not rows.empty:
                ...
                def _norm_label(lbl: str) -> str:
                    if lbl in ("T", "Theme"): return "Theme"
                    if lbl in ("C", "Coda"):  return "Coda"
                    m2 = re.match(r"V(\d+)", lbl, re.IGNORECASE)
                    return f"Var.{int(m2.group(1)):02d}" if m2 else lbl

                lbl_list = [_norm_label(l) for l in lbl_list]
                if len(lbl_list) >= 2:
